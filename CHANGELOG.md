...
- Saving a preference, renaming a camera, reordering the grid, creating or editing a group, and saving or applying a connection change no longer hang indefinitely when the server stops answering part-way through. These actions were sent with no time limit at all, so a request that stalled — a phone waking onto a network that is not quite back, the server restarting underneath it — left the spinner turning with no error, nothing to retry, and no way out short of reloading the app. These actions now give up after ten seconds. All of them except applying a connection change then report the failure the same way any other failure is reported, so you can see what happened and try again. Applying has the same ten-second limit, but running out of it is not reported as a failure: by then Skua may well be restarting, so the app waits for it to come back instead. The connection editor's Test button waits longer, fifteen seconds, because the server checks Frigate and go2rtc one after the other rather than at the same time, and cutting it short would abandon a check that was still running.
- Applying a connection change now waits for Skua to come back even when the confirmation of the restart never arrives. Skua confirms the restart and then immediately begins shutting down, so that confirmation is easily lost on the way — and a lost confirmation looked exactly like a failure: you were shown "Could not save" and the app then sat there, never noticing the server it had just restarted was already back. It now tells the two apart. A restart the server refuses outright still reports the reason it gave, unchanged. A restart with no answer at all is treated as one that may well have gone through, so the app keeps watching and reloads as soon as Skua answers again, with the same reminder after thirty seconds if it does not.
- Two README links now land where they should: the Setup guide link at the foot of the first-run setup page opens the setup instructions rather than the top of the README, where it fell because it pointed at a section that does not exist; and the Troubleshooting link on the page Skua shows when it cannot write to its data folder opens the section on that exact problem, the one the container log already names, rather than the general troubleshooting list.
...
- With the app installed to the home screen on an iPhone, the bar along the bottom sat clear of the bottom of the screen instead of against it, on any screen short enough not to scroll — a band of empty background showed underneath it, and it came back as soon as you moved to a screen that fitted. This was introduced by the 0.18.0 change that stopped short pages from scrolling when they fit: iOS holds the last stretch of the screen back from a page it thinks does not need it, and only hands it over once the page is asking for the full height. When it is installed to the home screen, the app now asks for the whole screen from the outset, and the bar sits against the bottom edge again everywhere. It is there from the first thing you see, too — an earlier attempt at this got the height back a moment too late, so on a cold start the bar appeared high and then slid down into place, which it no longer does. In a browser tab nothing changes: the bar was already flush there, and pages that fit still stay put.
...
- Screens that fit on one screen no longer scroll a little anyway. Every page was built to be at least as tall as the display, but the measurement used was the height the browser has when its address bar has slid away — so while the address bar was showing, every page in the app was taller than the space it had, and could be dragged up by that much to reveal nothing. The same went for the strip along the bottom of a phone where the home indicator sits: it was being reserved twice, so even installed to the home screen a page could be nudged up by a few tens of pixels. Pages now measure themselves against the space actually available, and a page that fits stays put.
