...
- With the app installed to the home screen on an iPhone, the bar along the bottom sat clear of the bottom of the screen instead of against it, on any screen short enough not to scroll — a band of empty background showed underneath it, and it came back as soon as you moved to a screen that fitted. This was introduced by the 0.18.0 change that stopped short pages from scrolling when they fit: iOS keeps the last stretch of the screen back from a page that cannot be scrolled, and hands it over the moment the page can be, so the scroll that change removed turned out to be the thing that was buying the app the full height of the display. The app now keeps a single pixel of scroll for itself when it is installed to the home screen, which is far too little to feel, and the bar sits against the bottom edge again on every screen. In a browser tab nothing changes: the bar was already flush there, and pages that fit still stay put.
