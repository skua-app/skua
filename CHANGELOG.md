...
- Two installs of the same Skua release are now the same Skua, and your device notices a new version because the app changed rather than because it was rebuilt. Every build stamped itself with the clock, so building the identical code twice produced two different images — the `main` and commit tags for one commit could point at different downloads, which makes "what am I actually running?" unanswerable and an upgrade harder to trust. The stamp now comes from the contents of the build instead, so one release is one set of files however many times it is built. Your phone or browser keeps a copy of the app's shell to start instantly and offline, and the marker that tells it the copy is out of date is derived from that same stamp: it changes whenever anything in the app changes, so upgrading still gives you the new version straight away, and re-pulling a release you already have no longer makes every device fetch the shell again for nothing.
