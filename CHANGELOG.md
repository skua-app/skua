...
- Held sideways, the recording timeline now hides its title bar and the navigation bar along the bottom, and puts a back control on the picture itself. The Follow/Fixed switch moves up to share the row with the playback controls, keeping the far end of that row to itself, rather than taking a whole row of its own with nothing beside it. All of that space goes to the recording instead of to chrome, and the playback controls stay centred exactly where they were. Held sideways on an iPhone whose camera notch moves to the side, the picture and the timeline no longer run underneath it: the screen now keeps that strip clear along both edges, so the earliest times on the track, and the back control on the picture, are no longer hidden behind the notch. The picture is letterboxed a little narrower to make the room, which is the trade — nothing is cropped, and nothing is left under the notch.
