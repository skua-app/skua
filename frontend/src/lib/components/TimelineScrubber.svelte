<script lang="ts">
  ...
  type Props = {
    ...
    // anchor expressed — consumer's choice". The scrubber only reports where the
    // gesture happened; whether to honour it is the consumer's decision (the
    // user's zoom-anchor preference lives there, not here).
    //
    // The wheel passes its pointer position. Pinch DELIBERATELY passes null in
    // this commit: its natural anchor is the midpoint between the fingers, but
    // touch behaviour is unchanged here, so it keeps zooming about the playhead
  ...
  function onPointerMove(e: PointerEvent) {
    ...
    if (pinching && activePointers.size === 2) {
      // Fingers apart -> currentDist up -> smaller span -> zoom in. null anchor:
      // pinch keeps zooming about the playhead this commit (see the onZoom prop
      // comment) — nothing about touch changes here.
